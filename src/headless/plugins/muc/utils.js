 ...
 */
export async function getAffiliationList (muc_jid, affiliation) {
 ...
 * @returns { Promise }
 */
export function setAffiliations (muc_jid, members) {
    const affiliations = [...new Set(members.map(m => m.affiliation))];
    return Promise.all(affiliations.map(a => setAffiliation(a, muc_jid, members)));
}
...
export function setAffiliation (affiliation, muc_jids, members) {
    ...
    members = members.filter(m => [undefined, affiliation].contains(m.affiliation));
    return Promise.all(muc_jids.reduce((acc, jid) =>
        [...acc, ...members.map(m => sendAffiliationIQ(affiliation, jid, m))], [])
    );
}
 ...
 * @namespace muc_utils
 */
export const muc_utils = { computeAffiliationsDelta }
