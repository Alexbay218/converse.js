...
import { muc_utils, setAffiliations, getAffiliationList }  from './utils.js';
...
const ChatRoomMixin = {
    ...
    async updateMemberLists (members) {
        ...
        const aff_lists = await Promise.all(all_affiliations.map(a => getAffiliationList(this.get('jid'), a)));
        const old_members = aff_lists.reduce((acc, val) => (u.isErrorObject(val) ? acc : [...val, ...acc]), []);
        await setAffiliations(muc_utils.computeAffiliationsDelta(true, false, members, old_members));
        await this.occupants.fetchMembers();
    },
